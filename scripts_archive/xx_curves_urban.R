...
# urban
urb = read.csv("./data/urban/UrbanStatusForRory.csv")
urb$species = tolower(unlist(lapply(strsplit(urb$Sp, "_"), paste, collapse=" ")))
urb = urb[ urb$UrbanBinary== 1, ]
...
vir = virion %>%
  ...
  dplyr::mutate(urban = Host %in% urb$species)
...
v1 = vir %>%
  ...
  dplyr::arrange(urban, YearEarliest)

# create cumulative curves of all years
curves_all = expand.grid(unique(v1$urban), 1930:endyear) %>%
  dplyr::rename("urban" = 1, "YearEarliest" = 2) %>%
  left_join(v1[ , c("urban", "YearEarliest", "Virus")]) %>%
  dplyr::group_by(urban, YearEarliest) %>%
  ...
  dplyr::mutate(Urban = ifelse(urban==TRUE, "Urban", "Non-urban"))

px = ggplot(curves_all) + 
  geom_line(aes(Year, VirusCumulative, group=Urban, col=Urban), size=1) + 
  theme_minimal() +
...
ggsave(px, file=paste("./output/urbancurves.png", sep=""), device="png", units="in", width=6, height=5, dpi=300)
...
v2 = vir %>%
  ...
  dplyr::group_by(HostOrder, urban, Virus) %>%
  dplyr::summarise(Database = paste(unique(Database), collapse=", "),
                   HostSpecies = paste(unique(Host), collapse=", "),
                   NumRecords = length(Year),
                   YearEarliest = min(Year, na.rm=TRUE),
...
# create cumulative curves of all years
curvesw = expand.grid(unique(v2$HostOrder), unique(v2$urban), 1930:endyear) %>%
  dplyr::rename("HostOrder" = 1, "urban" = 2, "YearEarliest" = 3) %>%
  left_join(v2[ , c("HostOrder", "urban", "YearEarliest", "Virus")]) %>%
  dplyr::group_by(HostOrder, urban, YearEarliest) %>%
  dplyr::summarise(Discovered = sum(!is.na(Virus)),
                   Virus = paste(unique(Virus), collapse=", ")) %>%
  left_join(v2[ !duplicated(v2$HostOrder), c("HostOrder", "VRichness") ]) %>%
  dplyr::arrange(desc(VRichness), HostOrder, urban, YearEarliest) %>%
  dplyr::group_by(HostOrder, urban) %>%
  dplyr::mutate(VirusCumulative = cumsum(Discovered)) %>%
  dplyr::rename("Year" = YearEarliest)

# separate out into domestic and wild
curves_urb = curvesw[ curvesw$urban == TRUE, ]
curves_wild = curvesw[ curvesw$urban == FALSE, ]
...
plot_data$DW = ifelse(plot_data$urban == TRUE, "Urban", "Non-urban")

# remove domestic curves from groups with no domesitcs
any_dom_orders = unique(vir$HostOrder[ vir$urban == TRUE])
plot_data = plot_data[ -which(plot_data$urban == TRUE & !plot_data$HostOrder %in% any_dom_orders), ]
...
px = ggplot(plot_data[ plot_data$Year <= 2010, ]) + 
  ...
  theme(strip.background = element_blank(), 
        ...
        axis.ticks = element_line()) + 
  ylab("Cumulative viral richness")
ggsave(px, file=paste("MS_MammalOrders_CumulativeCurves_VIRION_urban.png", sep=""), device="png", units="in", width=10, height=6.5, dpi=300)
...
for(i in 1:length(orders)){
  ...
  if(dd$HostOrder[1]=="chiroptera"){   m1 = mgcv::gamm(Discovered ~ s(Year), family=nb(), data=dd, method="REML") }
...
ggsave(curve_plot, file=paste("./output/figures_2021/MS_Figure1_OrderGAMs_", ictv_flag,".png", sep=""), device="png", units="in", width=8, height=7, dpi=600, scale=0.9)
