...
# ================== Compile databases with year of first detection ==================
...
eid = read.csv( "./output/data_processed/pathogens/EID2_Year_MammalsBirds_full.csv", stringsAsFactors = FALSE) %>%
...
assoc = read.csv("./data/host_pathogen_2020/data/hostparasitedb_raw/EID2/EID2_rorygibb/EID2_pathogendata_allpathogens_harmonised_20180916.csv", stringsAsFactors = FALSE) %>%
  select(Pathogen, PathogenName_Harmonised, PathogenType, HumanInfective_Any, DiseaseAgent, IsZoonotic, Disease_GIDEON, Route_GIDEON, UN_subregion, Countries_EID2Wertheim)
eid2 = left_join(eid2, assoc, by=c("Parasite"="Pathogen")) 
...
# save
write.csv(eid2, "./output/data_processed/hostpathogen_harmonised/EID2_MammalsBirds_Harmonised_Oct2020.csv", row.names=FALSE)
...
# main GMPD2 dataset
gmpd2 = read.csv("./data/host_pathogen_2020/data/hostparasitedb_raw/GMPD2/GMPD2_datafiles/GMPD_main.csv", encoding = "latin1", stringsAsFactors = F)
...
assoc = read.csv("./data/host_pathogen_2020/data/hostparasitedb_raw/GMPD2/GMPD2_rorygibb/GMPD2_pathogendata_allpathogens_harmonised_20180916.csv", stringsAsFactors = FALSE) %>%
  select(Pathogen, PathogenName_Harmonised, PathogenType, HumanInfective_Any, DiseaseAgent, IsZoonotic, Disease_GIDEON, Route_GIDEON, UN_subregion, Countries_EID2Wertheim)

# get data on missing pathogens from eid2
missing = gmpd2[ which(!gmpd2$Parasite %in% assoc$Pathogen), ]
missingrecs = read.csv("./data/host_pathogen_2020/data/hostparasitedb_raw/EID2/EID2_rorygibb/EID2_pathogendata_allpathogens_harmonised_20180916.csv", stringsAsFactors = FALSE) %>%
  select(Pathogen, PathogenName_Harmonised, PathogenType, HumanInfective_Any, DiseaseAgent, IsZoonotic, Disease_GIDEON, Route_GIDEON, UN_subregion, Countries_EID2Wertheim) %>%
  filter(Pathogen %in% missing$Parasite)
assoc = rbind(assoc, missingrecs)

# combine and save
gmpd2 = left_join(gmpd2, assoc, by=c("Parasite" = "Pathogen"))
write.csv(gmpd2, "./output/data_processed/hostpathogen_harmonised/GMPD2_Mammals_Harmonised_Oct2020.csv", row.names=FALSE)
...
oliv_assoc = read.csv("./data/host_pathogen_2020/data/hostparasitedb_raw/Olival/Olival_mammalviruses_datafiles/associations.csv", stringsAsFactors = F) %>%
  select(-WildDomInReference)
oliv_vir = read.csv("./data/host_pathogen_2020/data/hostparasitedb_raw/Olival/Olival_mammalviruses_datafiles/viruses.csv", stringsAsFactors = F)
oliv_mam = read.csv("./data/host_pathogen_2020/data/hostparasitedb_raw/Olival/Olival_mammalviruses_datafiles/hosts.csv", stringsAsFactors = F)
...
assoc = read.csv("./data/host_pathogen_2020/data/hostparasitedb_raw/Olival/Olival_rorygibb/Olival_pathogendata_allpathogens_harmonised_20180916.csv", stringsAsFactors = FALSE) %>%
  select(Pathogen, PathogenName_Harmonised, PathogenType, HumanInfective_Olival,  Zoonotic_Olival_Strict, HumanInfective_External, IsZoonotic, DiseaseAgent, Disease_GIDEON, Route_GIDEON) %>%
...
hp3 = left_join(hp3, assoc, by=c("Parasite"="Pathogen"))
...
# save
write.csv(hp3, "./output/data_processed/hostpathogen_harmonised/HP3_MammalViruses_Harmonised_Oct2020.csv", row.names=FALSE)
...
all_syns = read.csv("./data/host_pathogen_2020/data/hostparasitedb_raw/Synonyms_lookup_ITIS/hostdata_allsynonyms_itis_20180917.csv", stringsAsFactors=F, encoding="latin1")
all_syns1 = all_syns %>%
  group_by(Original) %>%
...
write.csv(assoc, "./output/data_processed/hostpathogen_harmonised/AllDatabases_Associations_Hosts_Harmonised_Oct2020.csv", row.names=FALSE)
