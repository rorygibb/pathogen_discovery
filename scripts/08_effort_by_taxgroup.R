...
#     dplyr::left_join(mam_o %>% dplyr::select(Order, SR) %>% dplyr::rename("HostOrder"=Order) %>% dplyr::mutate(HostOrder = tolower(HostOrder))) %>%
#     dplyr::left_join(
#       pubs %>% dplyr::filter(Year <= y) %>% dplyr::group_by(HostOrder) %>% dplyr::summarise(NumPubs = sum(NumPubs))
#     ) %>%
#     dplyr::mutate(NumPubs = replace(NumPubs, is.na(NumPubs), 0),
#                   VRich  = replace(VRich , is.na(VRich ), 0),
#                   VGGenusRich = replace(VGGenusRich, is.na(VGGenusRich), 0),
...
#     dplyr::rename("HostOrder" = Order) %>%
#     dplyr::left_join(
#       pubs %>% dplyr::filter(Year <= y) %>% dplyr::group_by(HostFamily) %>% dplyr::summarise(NumPubs = sum(NumPubs))
#     ) %>%
#     dplyr::mutate(NumPubs = replace(NumPubs, is.na(NumPubs), 0),
#                   VRich  = replace(VRich , is.na(VRich ), 0),
#                   VGGenusRich = replace(VGGenusRich, is.na(VGGenusRich), 0),
...
# aa = fam_df %>% 
#   dplyr::filter(HostOrder %in% c("Carnivora", "Artiodactyla", "Chiroptera", "Eulipotyphla", "Lagomorpha", "Perissodactyla",
#                                  "Primates", "Rodentia", "Didelphimorphia")) %>%
#   dplyr::mutate(logPubs = log(NumPubs + 1),
#                 logExpPubs = log(NumPubs_Expected + 1)) %>%
#   dplyr::filter(Year==2020)
# fam_plot = 
#   ggplot() + 
#   geom_point(data=aa, aes(log(SR), logPubs, col=HostOrder), size=3) +
#   geom_line(aes(log(SR), logExpPubs), data=aa %>% dplyr::select(-HostOrder), size=0.6, alpha=0.7) +
#   facet_wrap(~HostOrder) +
#   theme_bw() +
#   theme(legend.position="none")
# ggsave(fam_plot, file="./family_effort_plot.png", device="png", units="in", dpi=600, width=6, height=6)
# 
# 
# 
# # =================== year intervals =====================
...
#   
#   if(y == 1990){
#     vir_y = vir %>% dplyr::filter(Year <= 1990)
#   } else{
#     vir_y = vir %>% dplyr::filter(Year > (y-10) & Year <= y)
#   }
#   
#   # order level summary
#   vir_y_o = vir_y %>%
...
#     dplyr::left_join(mam_o %>% dplyr::select(Order, SR) %>% dplyr::rename("HostOrder"=Order) %>% dplyr::mutate(HostOrder = tolower(HostOrder))) %>%
#     dplyr::left_join(
#       pubs %>% dplyr::filter(Year %in% range(vir_y$Year)) %>% dplyr::group_by(HostOrder) %>% dplyr::summarise(NumPubs = sum(NumPubs))
#     ) %>%
#     dplyr::mutate(NumPubs = replace(NumPubs, is.na(NumPubs), 0),
#                   VRich  = replace(VRich , is.na(VRich ), 0),
#                   VGGenusRich = replace(VGGenusRich, is.na(VGGenusRich), 0),
...
#     dplyr::rename("HostOrder" = Order) %>%
#     dplyr::left_join(
#       pubs %>% dplyr::filter(Year %in% range(vir_y$Year)) %>% dplyr::group_by(HostFamily) %>% dplyr::summarise(NumPubs = sum(NumPubs))
#     ) %>%
#     dplyr::mutate(NumPubs = replace(NumPubs, is.na(NumPubs), 0),
#                   VRich  = replace(VRich , is.na(VRich ), 0),
#                   VGGenusRich = replace(VGGenusRich, is.na(VGGenusRich), 0),
...
#   dplyr::filter(HostOrder %in% c("Chiroptera", "Artiodactyla", "Rodentia", "Primates", "Lagomorpha", "Carnivora")) %>%
#   ggplot() + 
#   geom_point(aes(log(SR), logPubs, col=HostOrder), size=3) +
#   geom_line(aes(log(SR), logExpPubs)) +
#   facet_grid(HostOrder~Year) +
#   theme_bw()
# 
# fam_df %>% 
#   dplyr::mutate(logPubs = log(NumPubs + 1),
...
#   dplyr::filter(HostOrder %in% c("Chiroptera", "Artiodactyla", "Rodentia", "Primates", "Lagomorpha", "Carnivora")) %>%
#   ggplot() + 
#   geom_point(aes(log(SR), NumPubs, col=HostOrder), size=3) +
#   geom_line(aes(log(SR), NumPubs_Expected)) +
#   facet_grid(HostOrder~Year) +
#   theme_bw()
