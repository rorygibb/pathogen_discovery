...
# publiations
pubs = read.csv("./output/host_effort/PubMed_HostsEffort_PerYear_19302020.csv") %>%
...
# combine with virus curves
vir_pubs = left_join(pc, curves %>% dplyr::select(Host, Year, VirusCumulative) %>% dplyr::mutate(Host = Hmisc::capitalize(Host)))

# result at species level
# res_species = vir_pubs %>%
#   dplyr::filter(Year > 1969) %>%
#   dplyr::group_by(Year) %>%
#   dplyr::summarise(rho = cor.test(CumPubs, VirusCumulative, method = "spearman")$estimate,
#                    num_above_zero = sum(VirusCumulative > 0, na.rm=TRUE)/length(VirusCumulative)) %>%
#   dplyr::mutate(model = "Species")
...
p1 = rbind(reso, resf, ress) %>%
  ...
  ggplot() + 
  geom_line(aes(Year, rho, col=model), size=0.8) +
  theme_minimal() +
  xlab("Year") + ylab("Rank correlation between\ncitation count and viral richness") +
...
# p2 = resso1 %>%
#   ggplot() + 
#   geom_line(aes(Year, rho, col=HostOrder), size=0.8) +
#   theme_minimal() +
#   xlab("Year") + ylab("") +
#   theme(legend.title=element_blank(),
...
#         axis.text = element_text(size=11)) +
#   scale_y_continuous(limits=lims) + 
#   ggtitle("Species-level (by order)") +
#   theme(plot.title = element_text(size=14, hjust=0.5))

# 
# pc = gridExtra::grid.arrange(p1, p2, nrow=1, widths=c(1, 1.4))
# ggsave(pc, file="./output/cumpubs_viralrichness_cor.png", device="png", units="in", width=12, height=5)

ggsave(p1, file="./output/cumpubs_viralrichness_cor_spo.png", device="png", units="in", width=6, height=5)


# ============== how many obs/ how many ties =============

# rbind(reso, resf, ress) %>%
#   dplyr::mutate(model = factor(model, levels=c("Order", "Family", "Species"), ordered=TRUE)) %>%
#   ggplot() + 
#   geom_line(aes(Year, ties_vr/n_obs, col=model), size=0.8) +
#   theme_minimal() +
#   xlab("Year") + ylab("Rank correlation between\ncitation count and viral richness") +
#   theme(legend.title=element_blank(),
#         legend.position=c(0.85, 0.15),
