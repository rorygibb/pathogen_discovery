...
# Sensitivity check using Pearson coefficient (log-log) to ensure results are not overly sensitive to ties (driven by zero inflation in earlier years)
# Very similar results although correlations are slightly higher overall
# Still see the extremely rapid reshuffle within bats
...
# sensitivity test using parametric correlation (less sensitive to ties)
pearsonStability = function(year, data){
  ...
  # calculate coef
  rho = as.vector(cor.test(log(spx$ViralRichness+1), log(spx$ViralRichness_End+1), method="pearson")$estimate)
  return(rho)
}
...
stab = data.frame(Year = startyear:endyear, SpearmanCoef = sapply(startyear:endyear, pearsonStability, data=sps))
stab$Group = "Species"
...
stabo = data.frame(Year = startyear:endyear, SpearmanCoef = sapply(startyear:endyear, pearsonStability, data=spso))
stabo$Group = "Order"

curvesx = curves %>%
  dplyr::mutate(Host = HostFamily)
spsf = do.call(rbind.data.frame, lapply(startyear:endyear, getYearVRichness, data=curvesx))
stabf = data.frame(Year = startyear:endyear, SpearmanCoef = sapply(startyear:endyear, pearsonStability, data=spsf))
stabf$Group = "Family"
...
for(i in orders){
  ...
  stabx = data.frame(Year = startyear:endyear, SpearmanCoef = sapply(startyear:endyear, pearsonStability, data=vrx))
  stabx$Group = i
...
stab_oa = data.frame(Year = startyear:endyear, SpearmanCoef = sapply(startyear:endyear, pearsonStability, data=sps_oa))
stab_oa$Group = "Order"

# run for families
sps_fa = do.call(rbind.data.frame, lapply(startyear:endyear, getYearVRichnessAggregate, data=curves, aggregate = "family"))
stab_fa = data.frame(Year = startyear:endyear, SpearmanCoef = sapply(startyear:endyear, pearsonStability, data=sps_fa))
